...
var V = (t, e) => {
    for (var s in e) f(t, s, { get: e[s], enumerable: !0 });
  },
  S = (t, e, s, i) => {
    if ((e && typeof e == 'object') || typeof e == 'function')
      for (let r of O(e))
        !j.call(t, r) &&
          ...
          f(t, r, {
            get: () => e[r],
            enumerable: !(i = M(e, r)) || i.enumerable,
          });
    return t;
  };
var g = (t, e, s) => (
    ...
    S(
      e || !t || !t.__esModule
        ? f(s, 'default', { value: t, enumerable: !0 })
        : s,
      t
    )
  ),
...
var h = g(require('colors')),
  ...
  G = a.format.combine(
    ...
    a.format.printf(
      ({ timestamp: t, ms: e, level: s, message: i, stack: r }) => {
        let n = i;
        r &&
          (n += `
${r}`);
...
var k = async (t, e) => {
    ...
    for (let [i, r] of s)
      if (r.required && !t[i]) return `The field "${i}" has not been provided.`;
    for (let [i, r] of s)
      if (typeof t[i] !== r.type)
        return `The field "${i}" must be of type ${r.type}.`;
    for (let [i, r] of s) {
      if (r.options && !r.options.includes(t[i]))
      ...
      if (r.type === 'number') {
        if (
          ...
          return `The field "${i}" must be between ${r.min} and ${r.max}.`;
        if (r.min && typeof t[i] == 'number' && t[i] < r.min)
          return `The field "${i}" must be at least ${r.min}.`;
        if (r.max && typeof t[i] == 'number' && t[i] > r.max)
          return `The field "${i}" must be less than ${r.max}.`;
      }
      if (r.type === 'integer') {
        if (
          r.min &&
      ...
      if (r.type === 'string') {
        let n = r,
          o = t[i];
        if (r.min && r.max && (o.length < r.min || o.length > r.max))
          return `The field "${i}" must be between ${r.min} and ${r.max} characters.`;
        if (r.min && o.length < r.min)
          return `The field "${i}" must be at least ${r.min} characters.`;
        if (r.max && o.length > r.max)
          return `The field "${i}" must be less than ${r.max} characters.`;
        if (n.test === 'email' && !y(o))
      ...
      if (r.checks) {
        for (let n of r.checks) if (!(await n.run(i))) return `${n.response}.`;
      }
      if (r.type === 'boolean' && typeof t[i] != 'boolean')
      ...
      if (r.type === 'array') {
        ...
        if (r.items)
          for (let n of t[i]) {
            if (typeof n !== r.items.type)
              return `The field "${i}" must be an array of ${r.items.type}.`;
            if (!r.items.enum.includes(n))
  ...
  L = async (t, e, s) => {
    let i = await k(t, e);
    return i ? s.status(400).json({ status: 400, message: i }) : null;
  },
  c = L;
var C = require('mongoose');
var z = async (t, e, s, i) => {
    let r = await (0, C.startSession)();
    try {
      r.startTransaction(), await t(e, s, r), await r.commitTransaction(), i();
    } catch (n) {
      await r.abortTransaction(),
        m.error(n),
...
var p = class {
  ...
  addStringValue(e) {
    let s = new u('string');
    return e(s), (this.schema[s.name] = s), this;
  }
  addNumberValue(e) {
    let s = new u('number');
    return e(s), (this.schema[s.name] = s), this;
...
var l = class {
  ...
  setParamSchema(e) {
    let s = new p();
    return e(s), (this.paramSchema = s.schema), this;
  }
  setQuerySchema(e) {
    let s = new p();
    return e(s), (this.querySchema = s.schema), this;
  }
  setBodySchema(e) {
    let s = new p();
    return e(s), (this.bodySchema = s.schema), this;
  }
  setResponses(e) {
    ...
    (async () => {
      try {
        return !this.paramSchema && !this.querySchema && !this.bodySchema
          ...
          : (this.paramSchema && (await c(e.params, this.paramSchema, s))) ||
            (this.querySchema && (await c(e.query, this.querySchema, s))) ||
            (this.bodySchema && (await c(e.body, this.bodySchema, s)))
          ? void 0
          : this.controller(e, s, i);
      } catch (r) {
        m.error(r);
      }
    })();
...
var P = require('express'),
  d = class {
    ...
    addEndpoint(e) {
      ...
      let s = `${this.path}${e.path}`.replace('//', '/');
      switch (e.method) {
        case 'GET':
          this.raw.get(s, ...this.middlewares, e.controller);
          break;
        case 'POST':
          this.raw.post(s, ...this.middlewares, e.controller);
          break;
        case 'PUT':
          this.raw.put(s, ...this.middlewares, e.controller);
          break;
        case 'PATCH':
          this.raw.patch(s, ...this.middlewares, e.controller);
          break;
        case 'DELETE':
          this.raw.delete(s, ...this.middlewares, e.controller);
          break;
        case 'OPTIONS':
          this.raw.options(s, ...this.middlewares, e.controller);
          break;
        default:
    ...
    addEndpointFile(e) {
      for (let s of Object.values(e)) this.addEndpoint(s);
      return this;
    }
    access() {
...
var b = class {
  ...
  addRoute(e, s) {
    ...
    let i = /\/+/g,
      r = s.access();
    return (
      this.raw.use(e.replace(i, '/'), ...r.middlewares, s.raw, ...r.afterwares),
      this
    );
  }
  getRouter = () => this.raw;
